  ...
  constructor() {
    this.experiences = this.experiences.map((experience) => {
      ...
      const isSameDate: Boolean =
        experience.endDate.getFullYear() === today.getFullYear() &&
        experience.endDate.getMonth() === today.getMonth() &&
        experience.endDate.getDate() === today.getDate();
      const displayEndDate: string = isSameDate
        ? "Present"
        : format(experience.endDate, "MMM yyyy");
      const lengthOfServiceYears: number = differenceInYears(
        experience.endDate,
      ...
      experience.lengthOfService = `${format(
        experience.startDate,
        "MMM yyyy"
      )} - ${displayEndDate} . ${lengthOfServiceDisplay}`;
      return experience;
    });
  }
